...
\begin{document}
	...
	As the data volume is too big to analyze descriptor by descriptor, for all the frames and types of  Hamiltonians, we used these data in a Principal Component Analysis, with the biplots shown in \autoref{fig:descriptors_pca}. The first PCA contained all the data, for the four residues, all frames, Hamiltonians and local descriptors. For this first analysis we could eliminate some redundant descriptors of the analysis, resulting in the second round where the two first PCs keep high explained variance (~75\%), but with less variables. We can observe that the energy refinement Hamiltonians plays a insignificant roles, showing that the descriptor are small influence of the semiempirical type. For the residues, is possible to see how they are very different in reactivity, and how the glutamic acid and arginine can change its reactivity along the different coordinates.	  
	...
	\end{figure}
	 ...
	 In the third PCA round we remove the quantum chemical descriptors that are not associate directly with the CDFT, MEP and partial charge, and we can see how the variace in the objects associated with the arginine residues becomes only in the first PC. For these analysis we can draw that the arginine reactivity is modulated by the hard-hard interactions and the electrophilicity, which matches with the expected behavior of a resiude that will donate a proton and receive electron density in this process.

	Knowing from the PCA that the most relevant descriptors are the hardness for arginine and the nucleophilicity for the glutamate, we estimate the kernel density for these two variables to sample the structure with the most probable pair of these descriptor values in these residues. In \autoref{fig:descriptors_smp}, we show the graphical representation of this process two a biplot of the kernel density. 
	...
	With the distance and reactivity distribution analysis, we sample two frames from the molecular dynamics, as we show in \autoref{fig:descriptors_pc}. We depict the distance between the catalytic residues and the substrates, and for the structure sampled with the descriptors, the water molecule is more close to the substrate as the same time with the glutamic acid residue. In the coordinates sampled with the distance analysis, we observe that the water molecule is more closer to the arginine and more distant of the substrate. 
	
	\begin{figure}[H]
		\includegraphics[width=4in]{sampled_structures}
		\caption{Sampled frames with different methods. A) based on the most probable distances B) }
		\label{fig:descriptors_pc}			
	\end{figure}
	
	In the \autoref{fig:descriptors_struct}, we depict the netphilicity and local hardness descriptor for the two sampled structures. We see that the main difference is in the nucleophilicity in the adenine nitrogen that will receive the proton from arginine. In the frame sample with the reactivity descriptor, this nucleophilicity appears, but in the coordinates sample based on distance analysis the propensity is inverted. This is the main result, there are other interesting things that will be discussed in the poster section and when a full work with these results will be submitted for publication. 
	
	\begin{figure}[H]
		\includegraphics[width=4in]{descriptors_structural}
		\caption{ Reactivity maps depicting local hardness and netphilicity for the sampled structures. }
